        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 bg-white p-1 overflow-y-auto custom-scrollbar">
            <table className="w-full text-left text-[11px] border-collapse">
              ...
              <tbody>
                <tr onClick={() => setSelectedFile("MT6761_Android_scatter")} className={`group cursor-default ${selectedFile === "MT6761_Android_scatter" ? 'bg-[#cce8ff]' : 'hover:bg-[#e5f3ff]'}`}>
        ...
        <div className="p-3 bg-[#f0f0f0] border-t border-gray-200 space-y-3">
          <div className="flex items-center gap-4 text-[12px]">
            ...
            <div className="flex-1 flex">
               <input type="text" value={selectedFile || ""} readOnly className="flex-1 border border-gray-300 bg-white px-2 py-1 outline-none focus:border-blue-500" />
               <select className="border border-l-0 border-gray-300 bg-white px-1 outline-none"><option></option></select>
            </div>
          ...
          <div className="flex justify-end gap-2">
            <button onClick={onClose} className="px-8 py-1 bg-[#e1e1e1] border border-[#adadad] text-[12px] hover:bg-[#d5d5d5]">Open</button>
            <button onClick={onClose} className="px-8 py-1 bg-[#e1e1e1] border border-[#adadad] text-[12px] hover:bg-[#d5d5d5]">Cancel</button>
          </div>
        </div>
      </div>
    </div>
...
const DesktopIcon: React.FC<{ icon: React.ReactNode; label: string; onClick?: () => void }> = ({ icon, label, onClick }) => (
  <div onClick={onClick} className="flex flex-col items-center gap-1 p-2 w-20 h-20 rounded hover:bg-white/10 group cursor-default">
    <div className="p-1 group-active:scale-95 transition-transform text-white drop-shadow-md">
      {icon}
    </div>
    <span className="text-[11px] text-white text-center font-medium drop-shadow-[0_1px_1px_rgba(0,0,0,0.8)] leading-tight">
      {label}
    </span>
  </div>
);
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen relative win11-wallpaper overflow-hidden select-none">
      
      {/* Desktop Icons */}
      <div className="absolute top-4 left-4 flex flex-col gap-4">
        <DesktopIcon icon={<HardDrive size={36} className="text-blue-300" />} label="This PC" />
        <DesktopIcon icon={<Trash2 size={36} className="text-gray-300" />} label="Recycle Bin" />
        <DesktopIcon icon={<Chrome size={36} className="text-red-400" />} label="Google Chrome" />
        <DesktopIcon icon={<Folder size={36} className="text-yellow-400" />} label="Firmware Tools" />
        <DesktopIcon 
      ...
      {isWindowOpen && (
        <div 
          className={`fixed bg-[#f0f0f0] border border-[#a0a0a0] flex flex-col win-shadow rounded-sm overflow-hidden transition-all duration-200 ${isMinimized ? 'opacity-0 scale-90 translate-y-20 pointer-events-none' : 'opacity-100 scale-100'}`}
          style={{
        ...
        >
          ...
          <div 
            ...
            className="bg-white px-3 py-1 flex items-center justify-between border-b border-[#d0d0d0] cursor-default"
          >
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center">
              ...
              <button 
                ...
                className="p-2 hover:bg-[#e81123] hover:text-white transition-colors"
              >
                <X size={14} />
              </button>
            </div>
          ...
          <div className="flex-1 flex overflow-hidden">
            <div className="w-64 border-r border-[#d0d0d0] p-6 flex flex-col items-center bg-white">
              ...
              <div className="mt-6 w-full space-y-2 text-[11px] text-[#666]">
                  <div className="flex justify-between border-b border-gray-100 pb-1"><span>Chip Name:</span><span className="font-semibold text-gray-800 uppercase">MT6761</span></div>
                  <div className="flex justify-between border-b border-gray-100 pb-1"><span>Chip Version:</span><span className="font-semibold text-gray-800">0x00000000</span></div>
                  <div className="flex justify-between border-b border-gray-100 pb-1"><span>EMMC Flash:</span><span className="font-semibold text-green-600">Detected</span></div>
              </div>
            </div>
            <div className="flex-1 flex flex-col overflow-hidden bg-[#f0f0f0]">
              <div className="flex bg-[#f0f0f0] border-b border-[#d0d0d0] px-2 pt-2">
                {Object.values(TabType).map((tab) => (
                  <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-1 text-xs border border-b-0 rounded-t-sm mr-[2px] transition-colors ${activeTab === tab ? 'bg-white border-[#d0d0d0] relative z-10' : 'bg-[#e5e5e5] border-[#d0d0d0] hover:bg-gray-100'}`}>
                    {tab}
                  </button>
                ))}
              </div>
              <div className="flex-1 p-4 overflow-y-auto bg-white m-2 border border-[#d0d0d0] custom-scrollbar">
                {activeTab === TabType.DOWNLOAD && (
                  <div className="space-y-4">
                    <div className="flex items-center gap-6 pb-2 border-b border-gray-100">
                      <button onClick={handleFlash} disabled={isFlashing} className="flex flex-col items-center gap-1 group disabled:opacity-50">
                        <div className="p-2 rounded hover:bg-green-50 text-green-600 border border-transparent hover:border-green-200 flex flex-col items-center"><Download size={24} /><span className="text-[10px] uppercase font-bold text-gray-600 group-hover:text-green-600">Download</span></div>
                      </button>
                      <button onClick={handleStop} className="flex flex-col items-center gap-1 group">
                        <div className="p-2 rounded hover:bg-red-50 text-red-600 border border-transparent hover:border-red-200 flex flex-col items-center"><CircleStop size={24} /><span className="text-[10px] uppercase font-bold text-gray-600 group-hover:text-red-600">Stop</span></div>
                      </button>
                    </div>
                    <div className="space-y-2 text-[11px]">
                      <div className="grid grid-cols-[120px_1fr_80px] items-center gap-2"><span className="text-gray-600">Download-Agent</span><input type="text" readOnly value="ovatek\SP Flash Tool Version 5.1916 (Windows)\MTK_AllInOne_DA.bin" className="border border-[#ccc] px-2 py-0.5 bg-gray-50 outline-none" /><button onClick={() => openSelector("Open Download-Agent File")} className="flex items-center justify-center gap-1 border border-[#ccc] bg-gray-100 py-0.5 hover:bg-gray-200"><FolderOpen size={12} className="text-yellow-600" /><span>choose</span></button></div>
                      <div className="grid grid-cols-[120px_1fr_80px] items-center gap-2"><span className="text-gray-600">Scatter-loading File</span><input type="text" readOnly value="C:\Users\Hovatek\Desktop\SP Flash tool\Firmware\MT6761_Android_scatter.txt" className="border border-[#ccc] px-2 py-0.5 bg-gray-50 outline-none" /><button onClick={() => openSelector("Open Scatter File")} className="flex items-center justify-center gap-1 border border-[#ccc] bg-gray-100 py-0.5 hover:bg-gray-200"><FolderOpen size={12} className="text-yellow-600" /><span>choose</span></button></div>
                      <div className="grid grid-cols-[120px_1fr_80px] items-center gap-2"><span className="text-gray-600">Authentication File</span><input type="text" readOnly value="" placeholder="Optional" className="border border-[#ccc] px-2 py-0.5 bg-gray-50 outline-none" /><button onClick={() => openSelector("Open Authentication File")} className="flex items-center justify-center gap-1 border border-[#ccc] bg-gray-100 py-0.5 hover:bg-gray-200"><FolderOpen size={12} className="text-yellow-600" /><span>choose</span></button></div>
                      <div className="flex items-center pt-2"><select className="border border-[#ccc] text-[11px] px-1 py-0.5 outline-none bg-white min-w-[150px]"><option>Download Only</option><option>Firmware Upgrade</option><option>Format All + Download</option></select></div>
                    </div>
                    <div className="border border-[#ccc] overflow-x-auto">
                      <table className="w-full text-left text-[11px] border-collapse min-w-[800px]">
                        <thead className="bg-[#e1e1e1] text-gray-700"><tr><th className="p-1 border-r border-[#ccc] w-8"><input type="checkbox" checked readOnly /></th><th className="p-1 border-r border-[#ccc] w-24">Name</th><th className="p-1 border-r border-[#ccc] w-40">Begin Address</th><th className="p-1 border-r border-[#ccc] w-40">End Address</th><th className="p-1 border-r border-[#ccc] w-40">Region</th><th className="p-1">Location</th></tr></thead>
                        <tbody>
                          {partitions.map((part) => (
                            <tr key={part.id} className={`border-b border-[#eee] ${part.enabled ? 'bg-[#3cb371] text-white' : 'bg-white opacity-60'}`}>
                              <td className="p-1 border-r border-[#ccc] text-center"><input type="checkbox" checked={part.enabled} onChange={() => togglePartition(part.id)} /></td>
                ...
                {activeTab !== TabType.DOWNLOAD && (
                  <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-2"><Info size={48} /><p className="text-sm font-medium">Feature not implemented</p></div>
                )}
              </div>
              <div className="px-2 pb-1">
                 <div className="h-6 w-full bg-[#ddd] border border-[#bbb] relative overflow-hidden">
                    <div className={`h-full transition-all duration-300 ${isFlashing ? 'bg-yellow-400' : progress === 100 ? 'bg-green-500' : 'bg-gray-300'}`} style={{ width: `${progress}%` }}></div>
                    <div className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-gray-800 drop-shadow-sm">{isFlashing ? `[recovery] Download Flash ${Math.floor(progress)}%` : progress === 100 ? '[recovery] Download Flash 100%' : ''}</div>
                 </div>
              </div>
            </div>
          </div>
          <div className="bg-[#f0f0f0] border-t border-[#d0d0d0] px-3 py-1 text-[11px] text-gray-700 grid grid-cols-[100px_100px_150px_80px_1fr] divide-x divide-gray-300 items-center">
            ...
            <div className="px-2 font-bold flex items-center gap-1">EMMC <span className="font-normal text-gray-500 ml-1">High Speed</span></div>
            <div className="px-2">0:06</div>
            <div className="px-2 truncate">MediaTek PreLoader USB VCOM Port (COM48)</div>
          </div>
        </div>
      )}

      {/* Modals */}
      <FileSelector isOpen={isFileSelectorOpen} onClose={() => setIsFileSelectorOpen(false)} title={selectorTitle} />
      <SuccessDialog isOpen={isSuccessDialogOpen} onClose={() => setIsSuccessDialogOpen(false)} />

      {/* Windows 11 Centered Taskbar */}
      <div className="fixed bottom-0 left-0 w-full h-12 taskbar-glass flex items-center justify-center border-t border-white/20 z-[100]">
        
        {/* Centered App Group */}
        <div className="flex items-center gap-1.5 h-full py-1">
          {/* Start Icon */}
          <button className="h-10 w-10 flex items-center justify-center hover:bg-white/50 rounded transition-colors">
            <Shapes size={24} className="text-blue-500" />
          </button>
          ...
          {/* Active App: SP Flash Tool Icon */}
          <button 
            onClick={toggleWindow}
            className={`relative h-10 w-10 flex items-center justify-center rounded shadow-sm border transition-all ${isWindowOpen && !isMinimized ? 'bg-white/60 border-white/40' : 'hover:bg-white/40 border-transparent'}`}
          >
            <Monitor size={22} className="text-blue-800" />
            {isWindowOpen && (
              <div className={`absolute bottom-1 w-1.5 h-0.5 rounded-full transition-all ${!isMinimized ? 'bg-blue-600 w-3' : 'bg-gray-500'}`}></div>
            )}
          </button>
        ...
        {/* Right System Tray */}
        <div className="absolute right-3 flex items-center h-full gap-3 px-2">
            <div className="flex items-center gap-3 hover:bg-white/40 px-2 py-1 rounded transition-colors">
                <Wifi size={16} className="text-gray-700" />
                <Volume2 size={16} className="text-gray-700" />
                <Battery size={16} className="text-gray-700" />
            </div>
            <div className="flex flex-col items-end text-[12px] font-medium text-gray-800 px-2 leading-tight">
